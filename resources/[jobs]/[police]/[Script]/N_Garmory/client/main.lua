...
OpenWeaponStorage = function()
	...
	function(data, menu)
		...
		if IsEntityPlayingAnim(closestPed, "mp_police_armoury", "pistol_on_counter_police", 3) or IsEntityPlayingAnim(closestPed, "mp_police_armoury", "rifle_on_counter_police", 3) then
		...
		local animLib = "mp_police_armoury"

		LoadModels({ animLib })

		if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
			TaskPlayAnim(closestPed, animLib, anim .. "_on_counter_police", 1.0, -1.0, 1.0, 0, 0, 0, 0, 0)
