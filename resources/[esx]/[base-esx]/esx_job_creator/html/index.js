...
const DATATABLE_LANGUAGE = {
	lengthMenu: getLocalizedText("menu:datatable:lengthMenu"),
	zeroRecords: getLocalizedText("menu:datatable:zeroRecords"),
	info: getLocalizedText("menu:datatable:info"),
	infoEmpty: getLocalizedText("menu:datatable:infoEmpty"),
	infoFiltered: getLocalizedText("menu:datatable:infoFiltered"),
	search: getLocalizedText("menu:datatable:search"),
	paginate: {
		next: getLocalizedText("menu:datatable:next"),
		previous: getLocalizedText("menu:datatable:previous"),
	},
}
...
$("#edit-rank-form").submit(function(event) {
	...
	let data = {
		...
		rankGrade: $("#edit-rank-grade").val(),
		rankSalary: $("#edit-rank-salary").val(),
...
$("#create-rank-form").submit(function(event) {
	...
	let data = {
		...
		rankGrade: $("#create-rank-grade").val(),
		rankSalary: $("#create-rank-salary").val(),
		jobName: jobName
	}
...
function fillJobs(jobs) {
	...
	for (const [_, job] of Object.entries(jobs)) {
		jobsDataTable.row.add( [job.label, job.name, job.ranks.length] );
	}
...
$("#settings").submit(function(event) {
	...
	let serverSettings = {
		...
		isDiscordLogActive: $("#settings_isDiscordLogActive").prop("checked"),
		discordWebhook: $("#settings_discordWebhook").val(),
...
function loadSettings(fullConfig) {
	...
	$("#settings_isDiscordLogActive").prop("checked", fullConfig.isDiscordLogActive);
	toggleDiscordLogsInSettings(fullConfig.isDiscordLogActive);

	$("#settings_discordWebhook").val(fullConfig.discordWebhook);
	...
	$("#settings_cleanVehicleRemoveOnUse").prop("checked", fullConfig.repairVehicleRemoveOnUse);
